...
add_action('wp_head','flatsome_product_summary_fix');
...
add_action( 'woocommerce_share', 'flatsome_product_share',  0 );
...
function flatsome_product_lightbox_button(){ 
   if(flatsome_option('product_lightbox') !== 'disabled') { ?>
...
function flatsome_custom_product_tabs( $tabs ) {
  ...
  if(flatsome_option('tab_title')){
      $tabs['ux_global_tab'] = array(
        'title'   => flatsome_option('tab_title'),
        'priority'  => 50,
...
function flatsome_global_tab_content() {
  // The new tab content
  echo do_shortcode(flatsome_option('tab_content'));
}


function flatsome_product_tabs_classes(){
    $classes = array('nav','nav-uppercase');
    $tab_style = flatsome_option('product_display');
    ...
    $align = flatsome_option('product_tabs_align');
...
function flatsome_before_add_to_cart_html(){
    echo do_shortcode(flatsome_option('html_before_add_to_cart'));
}
...
function flatsome_after_add_to_cart_html(){
    echo do_shortcode(flatsome_option('html_after_add_to_cart'));
}
