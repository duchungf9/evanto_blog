...
function block_shortcode($atts, $content = null) {
    ...
    $post_id = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE $where_col = '$id'" );
    ...
    if ( $post_id ) {
        ...
        if ( current_user_can( 'edit_pages' )
           ...
           $html = '<div class="block-edit-link" data-title="Edit Block: '.get_the_title($post_id).'"   data-backend="'.$edit_link_backend.'" data-link="'.$edit_link.'"></div>'.$html.'';
        }
    } else {
