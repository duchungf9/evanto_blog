...
<!-- Custom CSS Codes -->
<style id="custom-css" type="text/css"><?php 
// Layout backgrounds
...
.container, .row{max-width: <?php echo $site_width - 30; ?>px}
...
.header-bg-image {background-repeat: <?php echo get_theme_mod('header_bg_img_repeat') ?>;}
<?php } ?>
...
echo 'h1,h2,h3,h4,h5,h6,.heading-font{font-weight: '.($type_headings['variant']*1).';}';
}
...
echo '.alt-font{font-weight: '.($type_alt['variant']*1).';}';
}
?>
...
<?php if(get_theme_mod('color_checkout')) { ?>
  [data-icon-label]:after, .button#place_order,.button.checkout,.single_add_to_cart_button.button{background-color: <?php echo get_theme_mod('color_checkout'); ?> }
<?php } ?>
...
<?php if(get_theme_mod('button_radius') && get_theme_mod('button_radius')  !== '0px') { ?>
 input[type="button"], button, .button{border-radius: <?php echo get_theme_mod('button_radius');?>}
<?php } ?>
